...
#' @param randomEffect character indicating grouping variable for random effect. only implemented as a random effect on the intercept.
...
analyzeComparative <- function(obj, rnaDesign, dnaDesign=NULL, fit.se=FALSE,
                                reducedDesign=NULL, correctControls=TRUE, randomEffect=NULL,
    ...
    if ("mode" %in% slotNames(obj) & (mode == "scale")) {
        ...
        if (!is.null(randomEffect)) {
            warning("Random effect model is not currently supported in scalable mode")
        }
    ...
    obj@designs@rnaFull <- getDesignMat(design=rnaDesign,
                                        annotations=rnaAnnot(obj))
    if (!is.null(randomEffect)) {
        obj@designs@randEff <- getRandomEffectMat(randomEffect = randomEffect,
                                                  annotations = rnaAnnot(obj))
    } else {
        obj@designs@randEff <- NULL
    }
    ...
    if(correctControls & any(controls(obj))) {
        if (!is.null(randomEffect)) {
            warning("Random effect model is not currently supported with control-based correction")
            randomEffect <- NULL
        }
    ...
    models <- bplapply(rownames(dnaCounts(obj)), function(rn) {
        ...
        tryCatch({
            return(fit.dnarna.noctrlobs(
                ...
                rdesign.mat=obj@designs@rnaFull,
                d2rdesign.mat=obj@designs@dna2rna,
                randeff.mat=obj@designs@randEff,
